package kr.ac.cnu.computer.stepcounter;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        alert.setTitle("목표 걸음수를 입력해주세요.");
        alert.setMessage("오늘 목표!");

        final EditText goal_step = new EditText(this);
        alert.setView(goal_step);

        alert.setPositiveButton("ok", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                String username = goal_step.getText().toString();
                goal = Integer.parseInt(username);

            }
        });

        now = walkingCount/goal*100;
        ...
        btn_start.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                thread.start();
                ProgressBarThread ProgressBarThread1 = new ProgressBarThread();
        ...
        public void run() {
            ...
            while (true) {
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        circleProgressBar.setProgress(now);
                    }
                });
